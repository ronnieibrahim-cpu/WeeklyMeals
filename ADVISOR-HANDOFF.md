...
37. **Cross-week rotation is real, and the M5.0 archive is now a scoring input (Ronnie, 2026-07-25) — this REVERSES an M5.0 accept criterion, deliberately.** M5.0 specced the rolling per-device archive as "strictly a reflection surface … no scoring input." Ronnie reported the symptom that forced the question: with one week of history, three hearted recipes all landed in the following week's menu. Root cause was that `WEIGHTS.favorite` was a flat, permanent +1.0 (its own comment claimed "periodic reintroduction"; no periodicity was ever implemented) and `GenerateContext` had no memory beyond the week being built — the gap AUDIT.md logged as B7/P1.2, accepted for Milestone 2 and never scheduled. Options put to Ronnie: (a) read the per-device archive + active plan, (b) current week only, (c) add multi-week cooked-history to sync. **He chose (a).** Rationale: it is the only multi-week memory that exists — ratings are NOT synced, and a plan carries only its own week — and generation happens on one phone whose result then syncs as a unit, so no merge is involved and Law #6 is untouched. Accepted consequences, both stated to him: rotation memory is per-device, and it is empty on a fresh install (everything reads as rested). Implementation: `src/engine/rotation.ts` (pure) → favorite rest ramp (3 weeks), per-week favorite crowding taper (2), repeat penalty fading over 4 weeks, MAINS ONLY (applying it through `scoreSide` would repeat decision 23's scope inversion — a weekly rice pilaf is not a defect). `WEIGHTS.favorite` 1.0 → 0.8 so even a rested favorite sits below `preference`, `affinity` and `variety`; `repeat` = 1.0, held below `variety` — both asserted as tests per decision 27's contract. Learned POSITIVE signals (affinity + dials) are additionally scaled by `mealsRated` (full strength at 12); `ratingsPenalty` and blocking are deliberately NOT damped — dislikes are believed immediately, likes have to earn standing. Evidence, `scripts/checkRotation.ts` (100 two-week scenarios): favorites returning to the very next week 2.56 → 0.01 of 3, week-over-week repeats 3.49 → 0.01 of 5, rested favorites still return more than un-hearted dishes (2.56 vs 1.65 — pacing, not suppression), 0 weeks left short.

38. **Re-roll near-misses now appear ALONGSIDE a thin strict list (Ronnie, 2026-07-25) — this changes the SHAPE of Product Law #2, deliberately.** Law #2 and M2.2's accept criteria both read "if nothing qualifies, show near-misses"; the mutually-exclusive branch was in all three code paths. Ronnie reported the offered pool being routinely three, worst before the shopping list settles. Three causes found: the cap (an unargued "top ~5" from the M2.2 spec, no rationale recorded anywhere), no cuisine diversification, and — the big invisible one — `composeSides` picking the best-scoring side with no notion of what's on hand, disqualifying a perfectly cookable main whenever its winning side needed something absent. Options put to him: (a) show near-misses alongside, clearly separated, (b) keep empty-only and just widen the strict pool, (c) alongside only below ~3. **He chose (a);** implemented with the threshold at 5 so a genuinely good week never shows them. Law #2's substance is explicitly preserved and remains non-negotiable: cookable-now options come first and primary, anything needing a shop is labeled with exactly what's missing, and choosing one still never edits the shopping list (Law #1). Also shipped: sides composed from on-hand sides only (all three modes), cap 5 → 8, cuisine diversification, near-miss allowance 3 → 5, and corrected screen copy (the old line promised strict-only offers, which would have become false). Keep-main mode keeps the empty-only fallback — it offers at most 3 plates for a fixed main by design, so the alongside rule would degenerate to "always". Evidence, `scripts/checkRerollPool.ts` (60 mid-week scenarios): fully-cookable plates 0.5 → 0.9 per scenario, 0 scenarios shrank, 0 offered plates not fully cookable. **The absolute number is the finding worth carrying forward:** with an empty pantry the strict pool is structurally under one plate, so the labeled near-miss list — not the strict pool — is what actually makes a mid-week re-roll a choice.

39. **Instant Pot nights (Ronnie, 2026-07-25) — content, not scoring.** Ronnie asked for a weekly option to include an Instant Pot dinner or two. Investigation first: the library had ZERO pressure-cooker recipes (0 of 280 curated; 17 stray mentions among imports, which are excluded outright whenever any allergy is set), `Recipe` had no equipment field, and `profile.equipment` existed but was read by nothing. Options put to him: (a) author ~12-16 real Instant Pot mains, (b) tag convertible existing recipes and add pressure-cook notes, (c) wire it with a 5-6 recipe starter set. **He chose (a).** Shipped: 15 hand-authored mains across all 12 cuisines with real pressure times, stated release methods and honest wall-clock cook times; new optional `Recipe.equipment`; `IntakeAnswers.instantPotNights` (0/1/2, optional so older plans read as 0); and a reservation pass in `generate()` that fills those nights BEFORE the general fill so the answer is always acted on (Law #3), while still respecting every hard filter and never leaving a week short. Verified by `scripts/checkInstantPot.ts` — 48 varied weeks per request level with cook limits swept 30-90 min, delivered 48/48 at both 1 and 2 nights. **Open item for Ronnie:** `profile.equipment` is still not editable anywhere, so the question is asked of every household rather than only those who own the machine. Gating it properly needs an equipment editor on Profile — not built, deliberately, as it was outside what he asked for.

40. **Sides are gated on culinary fit, not just cuisine equality (Ronnie, 2026-07-26).** Ronnie reported gremolata being composed onto chicken piccata. Investigation found a structural defect, not a data error: `composeSides`' second slot fell through to "best-scoring sauce" whenever the plate was already complete, and the only affinity signal was `cuisineFitBonus` — literally `side.cuisine === main.cuisine`, a 0.4 nudge that other scoring terms routinely outweighed. Measured across 601 mains: a sauce landed on **431** plates, **274** of which were two sauces and no real side (Salsa Roja 286 placements, Tahini 241; tahini on Thai green curry, pesto + gremolata on chana masala). Cuisine equality could never have caught the reported case — gremolata IS Italian, and so is piccata, which already carries a lemon-caper pan sauce. A second, compounding cause: `fitsCombinedTime` SUMMED the main's and side's cook times, so a 45-minute main against a 45-minute budget rejected every side with any cook time at all, leaving zero-cook sauces as the only eligible candidates on 90 plates. Options put to Ronnie: a per-sauce `pairsWith` allowlist vs. a 13×13 cuisine-affinity matrix; he approved the allowlist plus the time fix. Shipped: `saucePairsWithMain` (hard gate — main not already sauced, per `mainIsAlreadySauced` derived from existing `techniques`/`categories` so no main needed hand-editing; AND main's cuisine on the sauce's required `pairsWith`), failing closed, enforced at build time by `validateRecipes.ts`; cook time now parallel (`max`), prep still summed; and the identical gate applied to a KEPT sauce in `reroll.ts` (pairing gate (c)), which had assumed "a sauce pairs with any main" — the same bug through the component-re-roll door, and a textbook instance of product law #5. Result: sauced plates 431 → 82, two-sauce plates 274 → 21, and plates carrying a vegetable 547 → **577** (the parallel-cook fix put real sides where only sauces used to fit). **Ronnie explicitly scoped OUT** letting a starch attach to a main that already provides one ("should chana masala still get rice?") — a real open question, deliberately deferred: plates that already had a sensible side were left alone. **Known accepted consequence:** gremolata's classic home is the rich braise, and `mainIsAlreadySauced` blocks braises, so it now places only twice in the whole library — a rarely-placed sauce beats a wrongly-placed one, which is the complaint that started this. **Not advisor-reviewed** — approved by Ronnie in session; touches neither sync, allergy filtering, nor the shopping list.

41. **M5.7 recipe expansion + accuracy pass, and a diet-filter correction (Ronnie's request, 2026-09-23) — NOT yet advisor-reviewed; on branch `claude/recipe-expansion-accuracy-ths4k0`, not the deploy branch.** Ronnie asked for adversarial review/bug fixing, more recipes and variety (explicitly beef stroganoff), and a fidelity review of every recipe, with anything major held for him. Content: +65 curated mains (245 → 310) and corrections across ~200 of the 295 existing hand-written recipes (see MILESTONE-5.md M5.7) — the recurring defect was seasoning written into step prose as an aside ("a pinch of sugar balances it") and therefore absent from the ingredient list and the shopping list; now listed. This is the content-rewrite project decision 21 declined for the *plate* question; Ronnie's explicit ask here is the authority for it, and no recipe id changed. **Code change that touches diet filtering (owes the advisor pass per Law #4):** `satisfiesDiet` used to accept any `primaryProtein` of Eggs/Beans/Lentils/Tofu/None as vegetarian regardless of ingredients — Quiche Lorraine (bacon), agedashi tofu (dashi), an imported duck dish and a lard pastry all passed. Vegetarian is now `dietTags` only; pescatarian is fish/shellfish protein or a `vegetarian`/new `pescatarian` tag. The only curated main that stops qualifying is the quiche; five plainly vegetarian sides gained the tag they lacked. `normalize.ts`'s `inferAllergens`/`inferDietTags` were broadened in the only-ever-stricter direction (the sole allergen removals are the butternut-squash→"butter" false positive), `recipeImported.ts` regenerated from the frozen fixture with membership unchanged. Imports are still excluded outright whenever any allergy is set, so the practical effect is on diet tags (27 imported pasta/pastry dishes had been tagged gluten-free). Tests: a library-wide "nothing passing vegetarian lists meat" assertion (fails on the old code), keyword tests in both directions. **Deliberately NOT changed, awaiting Ronnie:** the shopping list's two-way substring pantry match (PROJECT.md §8) — a real bug, but fixing it changes what lands on the list (Law #1 territory) and needs a product call on what a pantry chip covers.

42. **Pantry matching: one shared allow-list rule (Ronnie, 2026-09-23, M5.8) — shopping-list law, NOT yet advisor-reviewed.** Four call sites used three rules: the shopping list and servings prompt a two-way substring ("Onions" dropped green onions, "Chicken" dropped chicken broth, "Canned tomatoes" dropped FRESH tomatoes while keeping crushed ones), re-roll a one-way substring (sausage counted as sage, egg noodles as eggs, peanut butter as butter), scoring the two-way one. Now all use `covers()` in `engine/pantryMatch.ts`. Ronnie's three calls, each the worker's recommendation: (1) a general word does NOT cover varieties ("Cheese" ≠ feta, "Beans" ≠ black beans, "Rice" ≠ arborio/basmati); (2) a specific item DOES cover the generic, but only through allow-listed same-item descriptors ("jasmine rice" ↔ "rice", "chicken thighs" ↔ "chicken"); (3) "Canned tomatoes" covers crushed/diced/fire-roasted — the one place the list now drops something the old rule kept. *Rationale — an allow-list, not an exclusion list, because the two errors are unequal:* a false "on hand" is found at the stove, a false "not on hand" is one line to tick off; so an unknown new word defaults to "buy it". Descriptor suffixes are per item, never global (a global "leaves" would make lime leaves cover limes). Evidence: `PANTRY-MATCH-REVIEW.md` (every pantry name × every curated ingredient, old vs new); re-roll on 60 seeded weeks 72 → 89 fully-cookable plates, the 7 single-plate losses all old false matches. Accepted consequence: re-roll is slightly stricter on generic recipe words ("beef" is not covered by ground beef).

43. **Optional ingredients stay on the shopping list; diet tags must hold for optional ingredients too (Ronnie, 2026-09-24, M5.8 task 2) — touches diet filtering and shopping-list content; NOT yet advisor-reviewed.** `optional: true` ingredients still reach the built list (option (a), an "add optional extras" button, was declined as larger than the problem). Instead `validateRecipes.ts` now fails any hand-authored recipe whose vegan/vegetarian/dairy-free/gluten-free tag is contradicted by an ingredient (optional ones included) or a declared allergen (`dietTagConflicts` in `normalize.ts`, tested in `normalize.test.ts`). It caught exactly three: `it-minestrone` and `me-mujadara` (tagged vegan + dairy-free, listed optional parmesan / yogurt and declared Dairy) — the garnish moved into a tip and Dairy was removed *because the only dairy ingredient is gone*, so a dairy allergy no longer blocks them; `it-spaghetti-bolognese` (tagged dairy-free, listed parmesan) — Ronnie chose to keep the parmesan and drop the `dairy-free` tag, so dairy-free diets no longer see it.

44. **Imported-recipe cleanup: lenient, refill, ship live (Ronnie, 2026-09-24, M5.8 task 3).** (Decision 43 — optional ingredients / diet tags, task 2 — is in PR #9 awaiting advisor review.) Ronnie's calls against the worker's recommendations (strict; no refill): **lenient** triage — fix what can be fixed, drop only broken recipes; **refill** — a dropped import's cuisine slot goes to the next-best candidate from the frozen fixture, for variety (each refill is reviewed in the same batch as the drop); and ship directly to the deploy branch. First piece shipped: British → US ingredient names and step wording, applied to the output only so no inference (allergens, diet tags, protein, department, spice) changes — verified by a field-by-field diff of all 356 imports. Content-only; no allergy, sync or shopping-list code changed, but the shopping list and pantry matching now see US words for imports (PANTRY-MATCH-REVIEW.md, curated-only, is unchanged).

45. **No refill for dropped imports; replace with hand-written recipes later (Ronnie, 2026-09-24, M5.8 task 3c).** Revises decision 44's refill call: the frozen TheMealDB fixture holds exactly 356 meals and every one is already in the app (the 45-per-cuisine cap never cut anything), and the environment's network blocks TheMealDB, so there is no next-best candidate. Options were (a) drop without refill (worker's pick), (b) allow TheMealDB and refresh the fixture, (c) drop now and replace the lost variety with hand-authored recipes later, alongside task 4. Ronnie chose **(c)**. Dropped ids go to `IMPORT_DROPS` in `src/data/import/importOverrides.ts` and are emitted as `recipeImportedRetired`: out of planning/re-roll/search/browse, but still resolvable by id so a saved plan keeps its dinner and shopping list (Law #1). Ids are never reused. **Outcome (2026-09-25):** all 356 triaged — 277 fixed, 8 kept, 71 dropped, 285 live (per cuisine: American 37, BBQ 18, Chinese 17, French 11, Greek 7, Indian 15, Italian 21, Japanese 6, Mediterranean 38, Mexican 10, MiddleEastern 39, Other 31, Thai 35). **Advisor review owed on label changes:** allergens only added except 7 removals, each because the triggering ingredient was removed — Eggs on 4 eggplant dishes (source spelled "Egg Plants"), Sesame on the Thai noodle salad (seeds belonged to the dropped steak variant) and the chivito sandwich (homemade-bun topping replaced by store buns), Gluten on Irish stew (optional wheat berries removed). Diet tags: 27 lose gluten-free (bread/pita/couscous now served), Irish stew gains gluten-free, one eggplant dish gains vegan. Imports are excluded entirely whenever any allergy is set, so these labels affect display and diet filtering, not the allergy guard.
